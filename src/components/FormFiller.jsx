...
import { buildFieldExtractionPrompt, buildFormQuestionPrompt, buildValidationPrompt } from '../utils/prompts'
import { validateField, normalizeAnswer } from '../utils/validators'

const FORM_TEMPLATES = [
  { id: 'rental', label: 'Rental Application', icon: '🏠', fields: [
  ...
  { id: 'medical', label: 'Medical Intake', icon: '🏥', fields: [
  ...
  { id: 'benefits', label: 'Benefits / Aid Application', icon: '📋', fields: [
...
export function FormFiller() {
  const [stage, setStage] = useState('select') // select | filling | review | done
  ...
  const { saveFormProgress, getFormProgress } = useOfflineCache()
  const inputRef = useRef(null)

  const currentField = fields[currentIndex]
  const answeredCount = Object.keys(answers).length

  // Ask the current field question
  const askQuestion = useCallback(async (index, fs, ans) => {
    const field = fs[index]
    if (!field) return
    const previousAnswers = fs.slice(0, index).map(f => ({
      label: f.label,
      value: ans[f.id] || ''
    })).filter(a => a.value)
    ...
    try {
      const { text } = await call({ ...prompt.systemPrompt && { systemPrompt: prompt.systemPrompt }, userPrompt: prompt.userPrompt })
      setQuestion(text)
  ...
  // Handle custom form upload
  const handleDocumentExtracted = useCallback(async ({ mode, text, base64, mimeType }) => {
    setExtracting(true)
    try {
      const prompt = buildFieldExtractionPrompt(text || '[Image attached]')
      ...
      if (extractedFields.length === 0) throw new Error('No fields found in document.')
  ...
  const submitAnswer = useCallback(async (value) => {
    ...
    if (!v && currentField?.required) {
      setValidationMsg('This field is required. Please enter an answer.')
      speak('This field is required. Please enter an answer.')
      return
    }
    ...
    setValidationMsg('')
    const normalized = normalizeAnswer(currentField.type, v)
    ...
    if (nextIndex < fields.length) {
      ...
      setStage('review')
      speak('Great job! Please review your answers before we finish.')
    }
    ...
    return (
      <div className="space-y-6 animate-fade-up">
        <div>
          ...
          <p className="text-white/50 text-sm">Choose a form below or upload your own. I'll ask you one question at a time.</p>
        </div>
        ...
        <div className="grid gap-3">
          {FORM_TEMPLATES.map(tmpl => (
            <button
              ...
              `}
              aria-label={`Use ${tmpl.label} template`}
            >
              ...
              <div>
                ...
                <p className="text-white/40 text-xs mt-0.5">
                  {tmpl.fields ? `${tmpl.fields.length} questions` : 'Upload your PDF or image'}
                </p>
              </div>
              <svg className="w-4 h-4 text-white/20 ml-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          ...
          <div className="animate-fade-up">
            {extracting ? (
              <div className="flex items-center gap-3 p-4 card">
                <div className="w-6 h-6 border-2 border-amber border-t-transparent rounded-full animate-spin flex-shrink-0" aria-hidden="true"/>
                <p className="text-white/60 text-sm">Gemma 4 is reading your form…</p>
              </div>
            ) : (
              <DocumentUploader
                label="Upload your form (PDF or photo)"
      ...
      <div className="space-y-6 animate-fade-up">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            aria-label={autoRead ? 'Turn off voice reading' : 'Turn on voice reading'}
        ...
        <div className="card p-5 space-y-4">
          {loading ? (
            <div className="flex gap-1.5 py-2" aria-label="Loading question" aria-live="polite">
              <span className="typing-dot" aria-hidden="true"/>
              <span className="typing-dot" aria-hidden="true"/>
              <span className="typing-dot" aria-hidden="true"/>
            </div>
          ) : (
            <p className="text-paper text-lg leading-relaxed font-body" aria-live="polite">{question}</p>
          )}
          ...
          {validationMsg && (
            <p role="alert" className="text-amber text-sm">{validationMsg}</p>
          )}

          {currentField?.helpText && (
            <p className="text-white/30 text-xs">{currentField.helpText}</p>
          )}
        </div>
          ...
          {fields.map((field, i) => (
            <div key={field.id} className="card p-4 flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-paper font-medium truncate">{answers[field.id] || <span className="text-white/20 italic">Not answered</span>}</p>
              </div>
              <button
                onClick={() => {
                  setCurrentIndex(i)
    ...
    return (
      <div className="flex flex-col items-center gap-6 py-8 text-center animate-fade-up">
        ...
        <div className="flex flex-col gap-3 w-full">
          ...
          <button onClick={() => { setStage('select'); setTemplate(null); setAnswers({}); setFields([]) }} className="btn-secondary w-full">Fill Another Form</button>
        </div>
      </div>
    )
  }
